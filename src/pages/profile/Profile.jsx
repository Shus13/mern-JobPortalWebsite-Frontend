...
const Profile = () => {
  ...
  const isEmployer = user?.role === "JobProvider";
  ...
  const handleResumeDownload = async () => {
    ...
    try {
      await downloadResume(`${user?.name || "resume"}-resume`);
    } catch (err) {
  ...
  return (
    <Container className="py-12">
      <PageTitle title="My Profile" subtitle="Manage your photo, resume, and account details." />

      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="p-6 lg:col-span-2">
          <h3 className="mb-5 font-display text-base font-semibold text-ink-900">
            {isEmployer ? "Company logo" : "Profile photo"}
          </h3>
          <AvatarUploader
          ...
          {isEmployer && (
            <p className="mt-3 text-xs text-ink-400">
              This photo appears next to your job postings so candidates recognize your company.
            </p>
          )}
          ...
          {user?.role === "JobSeeker" && (
            <>
              <h3 className="mb-4 font-display text-base font-semibold text-ink-900">Resume / CV</h3>
              <ResumeUploader
                currentResumePath={user?.resume}
